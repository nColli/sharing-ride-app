...
export default function StartTrip() {
  ...
  useEffect(() => {
    const startTrip = async () => {
      const url = getUrl() + "/api/trips/start/" + trip._id;
      ...
      try {
        ...
        const { alias, motivo, usersTo } = response.data;
        ...
        return newTripToSave;
      } catch (error) {
        console.log(error.response.data);
        setError("No hay viajes programados para dentro de una hora o menos");
      }
    };

    const fetchData = async () => {
      try {
        const nextTrip = await startTrip(auth); //si hay un viaje en media hora lo empieza y retorna el mismo, con la info para mostrar
      ...
      <View style={styles.container}>
        ...
        <View style={styles.tripContainer}>
              ...
              <>
                ...
                {reserves.map((reserve, index) => (
                  <Text key={reserve._id} style={styles.locationText}>
                    {index + 1}.{" "}
                    {reserve.placeStart.street +
                      " " +
                      reserve.placeStart.number +
                      ", " +
            ...
            {reserves.length > 0 && (
              <>
                ...
                {reserves.map((reserve, index) => (
                  <Text key={reserve._id} style={styles.locationText}>
                    {index + 1}.{" "}
                    {reserve.placeEnd.street +
                      " " +
                      reserve.placeEnd.number +
                      ", " +
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              router.push("/home/start-trip/finish-route/");
            }}
          >
