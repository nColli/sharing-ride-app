...
import { ScrollView, View, Text, TextInput, StyleSheet, Button, Alert, TouchableOpacity, Image } from 'react-native';
...
export default function VehicleRegistration() {
  ...
  const [seguroFoto, setSeguroFoto] = useState<string | null>(null);
  const [licenciaFoto, setLicenciaFoto] = useState<string | null>(null);

  const handleSubmit = async () => {
    if (!Object.values(formData).every(Boolean) || !seguroFoto || !licenciaFoto) {
      Alert.alert('Error', 'Completa todos los campos y sube las fotos requeridas');
      return;
    }
      ...
      {Object.keys(formData).map((field) => (
        <View key={field}>
          ...
          <TextInput
            ...
            keyboardType={['año', 'capacidad', 'kilometros'].includes(field) ? 'numeric' : 'default'}
